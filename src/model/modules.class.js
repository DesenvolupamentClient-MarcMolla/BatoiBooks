...
export default class Modules {
    ...
    populate(data) {
        this.data = data.map(item =>new Module(item.code, item.cliteral, item.vliteral, item.courseId));
    }

    addModule(module) {
        const lastId = this.data.reduce((maxId, module) => Math.max(maxId, module.code), 0);
        module.code = lastId + 1;
        module = new Module(module.code, module.cliteral, module.vliteral, module.courseId)
    ...
    getModuleByCode(moduleCode) {
        ...
        if (result) {
        return new Module(result.code, result.cliteral, result.vliteral, result.courseId);
        }
